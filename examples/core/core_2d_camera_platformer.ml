...
let update_player delta ({ position; speed; can_jump }, env_items, c, m) =
  ...
  in
  let speed =
    if is_key_down Key.Space && can_jump then ~-.player_jump_speed else speed
  in
  let player =
    if is_key_pressed Key.R then
      { position = half_screen; speed = 0.0; can_jump = true }
    else
      let rec player_collide_with = function
        | [] -> None
        | { blocking = true; box; _ } :: _
          when Rectangle.x box <= x
               && Rectangle.x box +. Rectangle.width box >= x
               && Rectangle.y box >= y
               && Rectangle.y box <= y +. (speed *. delta) ->
            Some (Rectangle.y box)
  ...
  let offset, target, mode =
    ...
    | EvenOutOnLanding (true, evening_target) ->
        ...
        let target_y, evening_out, evening_target =
          ...
          | Lower ->
              let target_y = target_y -. (evening_speed *. delta) in
              if evening_target >= target_y then (evening_target, false, 0.0)
              else (target_y, true, evening_target)
        in
        ( half_screen,
    ...
    | EvenOutOnLanding (false, _) ->
        let target_y = Vector2.y (Camera2D.target camera) in
        let start_moving =
          player.can_jump && player.speed == 0.0
    ...
    | PlayerBoundsPush ->
        ...
        let clamp_cam get_coord =
          if get_coord player.position > get_coord bbox_world_max then
            get_coord bbox_world_min +. get_coord player.position
            -. get_coord bbox_world_max
          else if get_coord player.position < get_coord bbox_world_min then
            get_coord player.position
